...
namespace OES.Blazor.Pages.Results
{
    public partial class TrackingLogs
    {
        ...
        private async Task<TableData<UnfinishedCandidateModel>> ServerReload(TableState state, CancellationToken token)
        {
            ...
            _paginationSearch.FilterObj = null;
            ...
            {
                _candidates = [];
                Snackbar.Add(Resource.NoTrackingLogsFound, Severity.Info);
            }
        ...
        private async Task ViewCandidateTrackingLogsDetails(UnfinishedCandidateModel candidate)
        {
            ...
            var dialog = await DialogService.ShowAsync<CandidateTrackingLogsDetailsDialog>(string.Empty, parameters, options);//to do
